...
public class Login extends Token implements Elements {
	private static final String TAG = "Login";
	public static final String PREF_NAME = "loginToken";

	public Login(String email, String password,Context context, Connection connection) {
		super(email, password, context, connection);
	}

	protected void getToken() throws IOException {
		Log.d(TAG, "Connecting...");
		InputStream in = connection.connect(DOMAIN + "/" + GET_LOGIN_TOKEN_URL,
		...
		try {
			LoginTokenParser p = new LoginTokenParser(in);
			String t = p.getToken();
			if (!t.equals(null)) {
				Log.d(TAG,"token " + t);
				token = t.trim();
